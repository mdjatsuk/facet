...
const { upload } = useApi()
const dragOver = ref(false); const busy = ref(false)
...
function showToast(type:'success'|'error'|'info', message:string){ toast.value={type,message}; setTimeout(()=>toast.value=null,3500) }
...
async function doUpload(file: File){
  ...
  } catch(e:any){ showToast('error', e?.data || e?.message || 'Üleslaadimine ebaõnnestus') }
  finally { busy.value = false }
}
function onInput(e: Event){ const input=e.target as HTMLInputElement; const f=input.files?.[0]; if(f) doUpload(f); input.value='' }
function onDrop(e: DragEvent){ dragOver.value=false; const f=e.dataTransfer?.files?.[0]; if(f) doUpload(f) }
</script>
<template>
  <div class="space-y-3">
